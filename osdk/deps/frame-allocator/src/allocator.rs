...
use super::set::FreeChunkSet;

/// The global free lists of buddy chunks.
static GLOBAL_POOL: SpinLock<FreeChunkSet<MAX_BUDDY_ORDER>, LocalIrqDisabled> =
    SpinLock::new(FreeChunkSet::new_empty());
static GLOBAL_POOL_SIZE: AtomicUsize = AtomicUsize::new(0);

// CPU-local free lists of buddy chunks.
cpu_local! {
    static LOCAL_POOL: RefCell<FreeChunkSet<MAX_LOCAL_BUDDY_ORDER>> = RefCell::new(FreeChunkSet::new_empty());
...
///
/// A maximum buddy order of 32 supports up to 16 TiBs of chunks.
const MAX_BUDDY_ORDER: BuddyOrder = 32;

/// Maximum supported order of the buddy system for CPU-local free lists.
///
...
///
/// A maximum local buddy order of 18 supports up to 1 GiBs of chunks.
...
pub mod balancing {
    ...
    use super::{
        lesser_order_of, BuddyOrder, FreeChunkSet, GLOBAL_POOL, GLOBAL_POOL_SIZE,
        MAX_LOCAL_BUDDY_ORDER,
    };
    ...
    pub fn balance(local: &mut FreeChunkSet<MAX_LOCAL_BUDDY_ORDER>) {
    ...
    fn balance_to<const MAX_ORDER1: BuddyOrder, const MAX_ORDER2: BuddyOrder>(
        a: &mut FreeChunkSet<MAX_ORDER1>,
        b: &mut FreeChunkSet<MAX_ORDER2>,
        order: BuddyOrder,
    ) where
        [(); MAX_ORDER1 as usize]:,
        [(); MAX_ORDER2 as usize]:,
