...
///
/// If this field remains 0 after initialization, we are not running on a
/// NUMA machine.
...
pub fn num_nodes() -> usize {
    // SAFETY: As far as the safe APIs are concerned, `NUM_NODES` is
    // read-only, so it is always valid to read.
    unsafe { NUM_NODES }
}

/// Returns whether we are running on a NUMA machine.
pub fn is_numa() -> bool {
    num_nodes() > 0
}

/// The processor affinity information for all CPUs.
pub static PROCESSOR_AFFINITIES: Once<&'static [ProcessorAffinity]> = Once::new();
/// The memory ranges associated with different proximity domains.
pub static MEMORY_RANGES: Once<&'static [MemoryRange]> = Once::new();
/// The NUMA distance matrix representing relative locality distances.
pub static DISTANCE_MATRIX: Once<&'static [u8]> = Once::new();

/// Initialize the NUMA topology.
pub fn init_numa_topology() -> Option<()> {
    let (num_nodes, processor_affinities, memory_ranges) = parse_srat_table()?;
    let (num_nodes, distance_matrix) = parse_slit_table(num_nodes);
...
/// Default distance within the same NUMA node, if not specified by SILT.
const DEFAULT_LOCAL_DISTANCE: u8 = 10;
/// Default distance between different NUMA nodes, if not specified by SILT.
const DEFAULT_REMOTE_DISTANCE: u8 = 20;

/// Returns the distance between NUMA nodes `node_i` and `node_j`, or `None` if
/// indices are out of bounds.
///
/// # Panics
///
/// This method panics if the NUMA topology hasn't been initialized.
pub fn distance(node_i: usize, node_j: usize) -> Option<u8> {
    let num_nodes = num_nodes();
    if node_i >= num_nodes || node_j >= num_nodes {
        return None;
    }

    let idx = node_i * num_nodes + node_j;

    DISTANCE_MATRIX.get().unwrap().get(idx).copied()
}
...
fn parse_srat_table() -> Option<(
    ...
    for range in memory_ranges.iter_mut() {
        ...
        max_end = max_end.max(range.base_address + range.length);
    }

    Some((num_nodes, processor_affinities, memory_ranges))
}

/// Parses the SLIT table and returns the total number of NUMA nodes and the
/// initialized NUMA distance matrix.
///
/// Falls back to default values if the SLIT table is missing.
fn parse_slit_table(num_nodes: usize) -> (usize, &'static mut [u8]) {
    ...
    let distance_matrix = default_distance_matrix(num_nodes);
...
/// Allocates and initializes a default NUMA distance matrix.
fn default_distance_matrix(num_nodes: usize) -> &'static mut [u8] {
    let distance_count = num_nodes.checked_mul(num_nodes).unwrap();

    // Allocate a region to store the distance matrix.
    let distance_matrix = {
        let layout = Layout::array::<u8>(distance_count).unwrap();
        let addr = allocator::early_alloc(layout).unwrap();
        let ptr = paddr_to_vaddr(addr) as *mut u8;
        ...
        // SAFETY: The memory is properly allocated and initialized. We exclusively own it. We
        // never deallocate it so it lives for `'static`. So we can create a mutable slice on it.
        unsafe { core::slice::from_raw_parts_mut(ptr, distance_count) }
    };
