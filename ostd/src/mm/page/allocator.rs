...
impl LocalCache {
    ...
    pub fn alloc(&mut self) -> Paddr {
        ...
        self.top -= 1;
        let paddr = self.cache_stack[self.top].take().unwrap();
        paddr
    }
    ...
    fn alloc_slow(&mut self) -> Paddr {
        let paddr = PAGE_ALLOCATOR
            .get()
            .unwrap()
            .lock_with(|allocator| allocator.alloc(LOCAL_LOW_WATERMARK).unwrap() * PAGE_SIZE);
...
pub(crate) fn alloc_single<M: PageMeta>(metadata: M) -> Option<Page<M>> {
    if !crate::INITIALIZED.load(Ordering::Acquire) {
        return PAGE_ALLOCATOR.get().unwrap().lock_with(|a| {
            a.alloc(1).map(|idx| {
...
pub(crate) fn dealloc_single(paddr: Paddr) {
    if !crate::INITIALIZED.load(Ordering::Acquire) {
        PAGE_ALLOCATOR
            .get()
            .unwrap()
            .lock_with(|a| a.dealloc(paddr / PAGE_SIZE, 1));
        return;
    }
