...
use ostd::{
    cpu::{CpuExceptionInfo, PinCurrentCpu},
    mm::{
        tlb::TlbFlushOp,
...
impl Vmar_ {
    ...
    pub(super) fn new_fork_root(self: &Arc<Self>) -> Result<Arc<Self>> {
        // Clone mappings.
        let mut new_vmspace = VmSpace::new();
        new_vmspace.register_page_fault_handler(handle_page_fault_wrapper);
...
impl<'a, R1, R2> VmarMapOptions<'a, R1, R2> {
    ...
    pub fn new(parent: &'a Vmar<R1>, size: usize, perms: VmPerms) -> VmarMapOptions<'a, R1, R2> {
        Self {
            parent,
            vmo: None,
            perms,
