...
pub trait ReadCString {
    ...
    /// stopping when a nul byte (`\0`) is encountered.
    ///
    /// Returns an error if no nul terminator is found before reaching
    /// `max_len` or the end of the reader.
    fn read_cstring_until_nul(&mut self, max_len: usize) -> Result<CString>;

    /// Reads a C string from `self` with a maximum length of `max_len`,
    /// stopping either at the first nul byte (`\0`) or when reaching the
    /// end of the reader.
    ///
    /// Returns the C string and the number of bytes read on success. If
    /// the reader ends before reaching `max_len`, and no nul byte is
    /// found, a nul byte is appended to ensure that the returned C string
    /// is valid.
    ///
    /// Returns an error if no nul terminator is found before reaching `max_len`.
...
impl ReadCString for VmReader<'_, Fallible> {
    ...
    fn read_cstring_until_end(&mut self, max_len: usize) -> Result<(CString, usize)> {
        let mut buffer: Vec<u8> = Vec::with_capacity(max_len);
        let origin_remain = self.remain();

        if read_until_nul_byte(self, &mut buffer, max_len)? {
            Ok((
                CString::from_vec_with_nul(buffer).unwrap(),
                origin_remain - self.remain(),
            ))
        } else {
            debug_assert!(buffer.len() < max_len);
            Ok((CString::new(buffer).unwrap(), origin_remain - self.remain()))
        }
    }
}
...
impl ReadCString for VmReaderArray<'_> {
    ...
    fn read_cstring_until_end(&mut self, max_len: usize) -> Result<(CString, usize)> {
        let mut buffer: Vec<u8> = Vec::with_capacity(max_len);
        let origin_remain: usize = self.sum_lens();

        for reader in self.readers_mut() {
            if read_until_nul_byte(reader, &mut buffer, max_len)? {
                return Ok((
                    CString::from_vec_with_nul(buffer).unwrap(),
                    origin_remain - self.sum_lens(),
                ));
            }
        }
...
mod test {
    ...
    fn read_multiple_cstring() {
        ...
        let read_str1 = reader.read_cstring_until_nul(100).unwrap();
        assert_eq!(read_str1, strs[0]);
        let read_str2 = reader.read_cstring_until_nul(100).unwrap();
        assert_eq!(read_str2, strs[1]);

        assert!(reader
            .read_cstring_until_nul(100)
    ...
    fn read_cstring_from_multiread() {
        ...
        let read_str1 = multiread.read_cstring_until_nul(100).unwrap();
        assert_eq!(read_str1, strs[0]);
        let read_str2 = multiread.read_cstring_until_nul(100).unwrap();
        assert_eq!(read_str2, strs[1]);
        let read_str3 = multiread.read_cstring_until_nul(100).unwrap();
