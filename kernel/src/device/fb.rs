...
impl FileIo for Fb {
    ...
    fn mmap(
        ...
        println!("Fb mmap: Not supported yet");
        Err(Errno::ENOSYS.into()) // Return "Operation not supported" error as a proper `Result`
    }
            ...
            IoctlCmd::GETVSCREENINFO => {
                ...
                if let Some(framebuffer_guard) = get_framebuffer_info() {
                    ...
                    let screen_info = FbVarScreenInfo {
                        ...
                        bits_per_pixel: framebuffer.bytes_per_pixel() as u32,
                    };
