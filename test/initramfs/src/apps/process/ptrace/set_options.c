...
#define PTRACE_SETOPTIONS_TEST(name, tracee_fn, eventmsg_cond)               \
	FN_TEST(ptrace_trace_##name)                                         \
	{                                                                    \
		...
		TEST_SUCC(ptrace(PTRACE_SETOPTIONS, pid, 0,                  \
				 PTRACE_O_TRACE##name));                     \
...
PTRACE_SETOPTIONS_TEST(EXEC, tracee_exec, eventmsg == pid)
...
PTRACE_SETOPTIONS_TEST(EXIT, tracee_exit,
		       WIFEXITED(eventmsg) && WEXITSTATUS(eventmsg) == 0)
