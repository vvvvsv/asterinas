...
FN_TEST(ptrace_tracer_exit_allows_tracee_progress)
{
	SKIP_TEST_IF(read_yama_scope() == YAMA_SCOPE_NO_ATTACH);
...
FN_TEST(ptrace_tracer_exit_reinjects_unwaited_signal)
{
	...
	if (tracer_pid == 0) {
		...
		CHECK(close(pid_pipe[1]));
		// Wait for the tracee to stop on the signal
		sleep(1);
		_exit(0);
	}
...
FN_TEST(ptrace_tracer_pid_traceme_detach)
{
	SKIP_TEST_IF(read_yama_scope() == YAMA_SCOPE_NO_ATTACH);
...
FN_TEST(ptrace_wait_nowait_preserves_ptrace_stop)
{
	SKIP_TEST_IF(read_yama_scope() == YAMA_SCOPE_NO_ATTACH);
