...
if [ "${NUMA}" = "true" ]; then
    ...
    if [ "$((SMP))" -eq 1 ]; then
        ...
        # If you have a qemu-system-x86_64 with NUMA binding support, you can use the following:
        # Add ",host-nodes=0,policy=bind" to the first object.
        # Add ",host-nodes=${HOST_NODE1},policy=bind" to the second object.
        NUMA_EXTRA_ARGS="\
