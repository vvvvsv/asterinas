...
## 第 7 页 — 2.3 tracer 与 tracee 如何同步：一把锁 + 一个原子标志

![同步机制](assets/04b_sync.png)

- 同步状态收敛在 `TraceeStatus = { state: Mutex<TraceeState>, is_stopped: AtomicBool }`——一把锁保护共享快照，一个原子标志做「停 / 跑」握手。
- **互斥锁 `state`**：tracer 的 wait、读写寄存器、resume 与 tracee 进入 stop 都「先拿锁」，让寄存器快照、待决信号、event 等共享状态不被并发读写。
- **原子标志 `is_stopped`**：它是「是否正停在 ptrace-stop」的唯一真相——tracee 进 stop 时置 true 后挂起（`pause_until` 轮询该标志），tracer 每次访问先 `check_ptrace_stopped`（未停止即返回 `ESRCH`），resume 时置 false 并唤醒 tracee，让它持锁读回快照继续运行。
...
## 第 9 页 — 2.5 x86-64 寄存器 ABI：字段级写策略
